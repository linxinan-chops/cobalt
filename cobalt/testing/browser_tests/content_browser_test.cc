...
void ContentBrowserTest::PreRunTestOnMainThread() {
  ...
  CHECK_EQ(TestShell::windows().size(), 1u);
  shell_ = static_cast<TestShell*>(Shell::windows()[0]);
...
TestShell* ContentBrowserTest::CreateBrowser() {
  return TestShell::CreateNewWindow(
      ShellContentBrowserTestClient::Get()->browser_context(),
      GURL(url::kAboutBlankURL), nullptr, gfx::Size());
}

TestShell* ContentBrowserTest::CreateOffTheRecordBrowser() {
  return TestShell::CreateNewWindow(
      ShellContentBrowserTestClient::Get()->off_the_record_browser_context(),
      GURL(url::kAboutBlankURL), nullptr, gfx::Size());
}
